...
def build_doc():
    ...
    for text in [
        ...
        "（3）基于 RAG 的知识问答模块。用户选择文件夹后，可在该范围及其子文件夹内提问。系统先执行混合检索，召回与问题相关的知识片段，再进行规则重排序和多样性筛选，最后构造包含资料片段和引用要求的 Prompt 调用大模型生成答案。系统支持答疑模式、教师模式、深度回答、来源引用和 SSE 流式输出；当 Elasticsearch、Embedding 或模型接口不可用时，系统提供本地关键词检索和摘要兜底。",
